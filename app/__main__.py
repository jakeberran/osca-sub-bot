...
from mainFuncs import readParseAndHandle, getCurrentSubRequests, updateSubRequests, writeAndSend, updateDatabasePath
...
def main():
  ...
  # Specify how many emails to read
  readNemails_ = ctk.StringVar(homeButtons, '')
  label_readNemails = ctk.CTkLabel(homeButtons, text='# of emails to read\n(blank for all unread)')
  entry_readNemails = ctk.CTkEntry(homeButtons, textvariable=readNemails_, placeholder_text='# emails to read')
  label_readNemails.grid(row=0, column=0, padx=20, pady=(10, 0))
  entry_readNemails.grid(row=1, column=0, padx=20, pady=10)
  ...
  def processNewEmails():
    ...
    try:
      ...
      tk.messagebox.showerror('Error', 'There was an error fetching your emails. Check your email and password.')
  ...
  button_processNewEmails.grid(row=1, column=1, padx=20, pady=10)
  button_sendRealEmail.grid(row=1, column=2, padx=20, pady=10)
  homeButtons.pack()
  ...
  def loadSettings():
    for i, s in enumerate(settings):
      ...
      initial_value = config(s['varname'], default=s['default'], cast=c)
  ...
  shifts = getDB('app/data/shifts.json')
  ...
  def addShiftTime(tab, listbox, byWhat, listName):
    ...
    # reload
    overwriteDB(shifts, 'app/data/shifts.json')
